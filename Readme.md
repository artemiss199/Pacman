...
## Overview & Features
The game architecture is built using several core classes: Game, Pacman, Ghost, Maze, Pellet, and ScoreManager. It includes a fully functional main menu that handles starting the game, exiting, selecting the game mode (difficulty), and selecting the level.

### Controls & Gameplay
* **Movement:** You can both control the Pacman using WASD keys or arrow keys.
* **Collision Rules:** Pacman and ghosts cannot bypass walls or the border defined in the map. You would stop moving on collision with walls, and ghosts choose a new direction on collision with walls. Only moving in allowed directions is permitted.
...
* **Collecting Beans:** Collecting beans using Pacman would lead in bean deletion and adds 10 points to the score.
...
* You can find the fully explained implementation in the `Ghost.java` file with explanation on the code.
...
* **Sound Effects (SFX):** All sound effects can be found in the `Assets/sfx` folder. We have sfx for the main menu, game start, Pacman eating beans, Pacman eating a ghost, and losing.